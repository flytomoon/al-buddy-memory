...
- All the operator's staff, contractors, vendors and integration partners who handle user data

**Foundational principle:** The user owns all data. The operator is the steward, not the owner. The AI interprets and organizes — it never owns. Default state is private. All sharing is explicit and revocable.
...
All data created by or about a user within the operator belongs to that user. This includes:
...
- **Conversations:** Full conversation histories are user-owned records, not the platform logs
...
The operator retains no ownership interest in any user data. The operator's operational use of data (described in Section 3) is a licensed, limited, revocable function — not ownership.
...
No default state may be "shared" or "opt-out." Every default is private. The operator does not use pre-checked consent boxes.
...
All user data at rest and in transit is encrypted. Encryption keys are user-controlled. The operator staff and systems do not hold master decryption keys for individual user data.
...
| Cross-device sync payloads | End-to-end encrypted before upload | User's key material; not decryptable by the operator |

#### Key Management

- User encryption keys are derived from user-controlled credentials (passkeys, device authentication)
- the operator does not hold, escrow, or have recovery access to user encryption keys
- Key rotation is supported and encouraged; the system provides tooling for key rotation without data loss
- If a user loses their key material, the operator cannot recover encrypted data — this is by design and must be disclosed to users at account setup

### 4.3 No Biometric Dependency

Authentication and encryption key derivation must never require biometric data. Passkey-based and device-credential authentication are the primary mechanisms. Biometrics may be used as a device-level convenience layer (e.g., Face ID to unlock the passkey store) but the operator systems never receive, store, or process biometric data directly.
...
The operator treats the right to forget as equal in importance to the right to remember. Users may delete:
...
Export includes: all memories, conversation history, learning records, AI behavior logs, and account metadata. Export does not include the operator's internal model weights or system configurations.
...
| **Profiling for external use** | Creating user profiles derived from the operator data for use outside the operator system |
...
| **Law enforcement disclosure without process** | Sharing user data with law enforcement without a valid legal order; the operator will not voluntarily comply with informal requests |
...
The operator does not share user data with third parties except in the following limited circumstances:
...
3. **Legal obligation:** A valid court order, subpoena, or regulatory requirement — the operator will notify affected users unless legally prohibited from doing so
...
The operator internal staff access to user data is restricted to:
...
No the operator employee, contractor, or agent may access individual user data for curiosity, business intelligence, product research, or any purpose not listed above.
...
The operator's monetization model is ethical subscription or lifetime license. The following monetization practices are prohibited:
...
- Any commercial arrangement that creates an incentive for the operator to collect more user data than the product requires
...
|---|---|---|
| **Right to access** | Receive a complete copy of all data the operator holds | In-app export or the maintainers (open an issue) |
...
The operator responds to all privacy rights requests within **5 business days** for acknowledgment and **30 days** for fulfillment.
...
Every person or system that designs, builds, configures, or operates any aspect of the operator that touches user data is obligated to comply with this policy. This includes external contractors, integration partners, and cloud infrastructure providers.

### 9.2 Vendor and Partner Requirements

Any vendor, partner, or infrastructure provider that handles the operator user data must:

1. Sign a data processing agreement that is consistent with this policy
2. Accept audit rights for the operator (or its designated auditor)
3. Notify the operator within 24 hours of any security event that may affect user data
4. Delete user data within 30 days of the operator's request
...
| **Key escrow** | A prohibited practice in which the operator would hold copies of user encryption keys — the operator does not do this |
| **User sovereignty** | The principle that the user is the ultimate authority over all data generated by or about them within the operator |
