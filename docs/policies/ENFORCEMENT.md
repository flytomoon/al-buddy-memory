...
| A governed read's page is the first facts the actor may see | `src/governance/governed-store.ts` | hidden facts never take a place on a limited page (so page counts cannot be used to probe for secrets), and a cursor the actor cannot see behaves as a missing one |
