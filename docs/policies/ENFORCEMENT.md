...
| Every governed decision is recorded — when an audit sink is supplied | `src/governance/audit.ts` | append-only events (allowed, hidden, denied). `ChainedAudit` hash-chains them (HMAC with a key): an edited, removed, inserted or reordered line is named by `verifyAuditChain` / `al-buddy-memory verify-audit`; a cut-off tail or a rewrite by the key holder is caught only against a head hash anchored elsewhere. The shipped MCP server writes a chained log per process (`<db>.audit/<start>-<pid>.jsonl`), because one chain has one writer and two assistants are two processes. The event is written after the store call succeeds: a sink that fails leaves **that one** committed write without its event, and the call rejects. "That one" is a bound, and the queue below is what makes it one — while `addNode` was off the queue, twenty concurrent adds all cleared the latch before the first failure was observed and all twenty landed (corrected 2026-09-19; `src/governance/audit-poison.test.ts`). The embedding cache follows the facts' visibility but is not audited (vectors are derived and never returned as facts) |
