...
| Every governed decision is recorded — when an audit sink is supplied | `src/governance/audit.ts` | append-only events (allowed, hidden, denied). `ChainedAudit` hash-chains them (HMAC with a key): an edited, removed, inserted or reordered line is named by `verifyAuditChain` / `al-buddy-memory verify-audit`; a cut-off tail or a rewrite by the key holder is caught only against a head hash anchored elsewhere. The shipped MCP server writes a chained log per process (`<db>.audit/<start>-<pid>.jsonl`), because one chain has one writer and two assistants are two processes. The event is written after the store call succeeds: a sink that fails leaves **that one** committed write without its event, and the call rejects. The embedding cache follows the facts' visibility but is not audited (vectors are derived and never returned as facts) |
| A store whose audit sink has failed stops changing | `src/governance/governed-store.ts` | the failure is latched on the sink, so every governed handle sharing it refuses the next write, update, erasure, link or import **before** touching the store, until the process restarts. This does not close the window above — it stops that window being reopened by a retrying client, which is what used to happen (R3, release review 2026-09-18; `src/governance/audit-poison.test.ts`) |
| Authorisation and the mutation it authorises are one step | `src/governance/governed-store.ts` | governed mutations over one store are serialised, so a policy decision cannot be committed against a row another handle changed while the (async) policies ran. One process only: two processes on one file have SQLite's write lock, which covers the write and not the decision (`src/governance/governed-race.test.ts`) |
| One project's facts are one project's | `src/project-memory.ts`, `src/sqlite-memory-store.ts` | the filename carries a digest of the whole scope, and the scope is recorded inside the database; opening a store under a different scope throws. Until 0.4.2 every character outside `[a-zA-Z0-9_-]` became `-`, so `org/repo` and `org-repo` shared one file and each recalled the other's private facts (R1, release review 2026-09-18; `src/project-memory-isolation.test.ts`) |
